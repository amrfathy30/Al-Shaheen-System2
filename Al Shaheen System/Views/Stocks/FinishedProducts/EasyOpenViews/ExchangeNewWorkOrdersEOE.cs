...
{
    public partial class ExchangeNewWorkOrdersEOE : Form
    {
       
        List<SH_SPECIFICATION_OF_EASY_OPEN> specList = new List<SH_SPECIFICATION_OF_EASY_OPEN>();
        CLIENT_ORDER_WORK clientOrder = new CLIENT_ORDER_WORK();
        public ExchangeNewWorkOrdersEOE(CLIENT_ORDER_WORK anyClientOrder)
        {
            InitializeComponent();
        ...
        long loadpcification()
        {
            ...
            try
            {
                ...
                string query = "SELECT* FROM SH_SPECIFICATION_OF_EASY_OPEN WHERE SH_RAW_MATERIAL_TYPE = @type AND SH_SIZE_NAME =@size AND SH_FIRST_FACE_NAME =@firstFace AND SH_SECOND_FACE_NAME = @secondFace AND SH_OPEN_WAY = @open";
                ...
                while (reader.Read())
                {
                    specList.Add(new SH_SPECIFICATION_OF_EASY_OPEN() { SH_ID = long.Parse(reader["SH_ID"].ToString()), SH_SIZE_NAME = reader["SH_SIZE_NAME"].ToString(), SH_CLIENT_PRODUCT_NAME = reader["SH_CLIENT_PRODUCT_NAME"].ToString(), SH_TOTAL_NO_ITEMS = long.Parse(reader["SH_TOTAL_NO_ITEMS"].ToString()), SH_CONTAINER_NAME = reader["SH_CONTAINER_NAME"].ToString(),SH_FIRST_FACE_NAME=reader["SH_FIRST_FACE_NAME"].ToString(),SH_CLIENT_NAME=reader["SH_CLIENT_NAME"].ToString(),SH_OPEN_WAY=reader["SH_OPEN_WAY"].ToString(),SH_SECOND_FACE_NAME=reader["SH_SECOND_FACE_NAME"].ToString() });
            ...
            {
                MessageBox.Show("ERROR WHILE GETTING EOE specification FROM DB " + ex.ToString());
            }
            return 0;
        }
        void fillPeelOffSpecifgridview()
        {
            ...
            if (specList.Count > 0)
            {
                ...
                {
                    dataGridViewEOEQnty.Rows.Add(new string[] { specList[i].SH_SIZE_NAME, specList[i].SH_RAW_MATERIAL_TYPE, specList[i].SH_FIRST_FACE_NAME, specList[i].SH_SECOND_FACE_NAME,specList[i].SH_OPEN_WAY });

                }
            }
        }
        private void ExchangeNewWorkOrdersEOE_Load(object sender, EventArgs e)
        {
