...
{
    public partial class Easyopenaddingform : Form
    {

        List<SH_EASY_OPEN_DATA> form_data = new List<SH_EASY_OPEN_DATA>();
        ...
        List<SH_SPECIFICATION_OF_EASY_OPEN> specifications = new List<SH_SPECIFICATION_OF_EASY_OPEN>();
        DatabaseConnection myconnection = new DatabaseConnection();

        private SH_EMPLOYEES mEmployee;
        private SH_USER_ACCOUNTS mAccount;
        private SH_USER_PERMISIONS mPermission;

        public Easyopenaddingform(SH_EMPLOYEES anyemp , SH_USER_ACCOUNTS myaccount , SH_USER_PERMISIONS myperm)
        {
            InitializeComponent();
            mEmployee = anyemp;
            mAccount = myaccount;
       ...
       async void fillsupplierbranchescombobox()
        ...
        async Task gettingproductsbyclientid()
        {
            ...
            else
            {
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("@SH_PRINTING_TYPE","إيزى أوبن");
        ...
        async Task autogeneratebottomaddtion_permission_number()
        {
            ...
            {
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand("SELECT (MAX(SH_ID)+1) AS lastedid FROM SH_ADDITION_PERMISSION_NUMBER_OF_EASY_OPEN  ", DatabaseConnection.mConnection);
                SqlDataReader reader = cmd.ExecuteReader();
        ...
        private void savenewpermssionnumber()
        {
            try
            {
                ...
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand("INSERT INTO SH_ADDITION_PERMISSION_NUMBER_OF_EASY_OPEN (SH_NUMBER) VALUES(@SH_NUMBER) ", DatabaseConnection.mConnection);
        ...
        private async void save_easy_open_btn_Click(object sender, EventArgs e)
        {
            ...
            Cursor.Current = Cursors.Default;
            this.Hide();
            using (Easyopenaddingform myform = new Easyopenaddingform(mEmployee, mAccount, mPermission))
            {
                myform.ShowDialog();
            }
            this.Close();
        ...
        void loadalleasyopenspecifications()
        {
            ...
            try
            {
                string query = "SELECT  SH_SPECIFICATION_OF_EASY_OPEN.* FROM SH_SPECIFICATION_OF_EASY_OPEN";
                DatabaseConnection myconnection = new DatabaseConnection();
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand(query , DatabaseConnection.mConnection);
                SqlDataReader reader = cmd.ExecuteReader();
        ...
        long checkifeasyopenspecificationsexistsornot( SH_EASY_OPEN_DATA mydata)
        {
            specifications.Clear();
        ...
        async Task updateeasyopenspecifications(long sp_id , SH_EASY_OPEN_DATA mydata)
        {
            try
            {
                string query = "UPDATE SH_SPECIFICATION_OF_EASY_OPEN SET SH_TOTAL_NO_ITEMS = SH_TOTAL_NO_ITEMS + @SH_TOTAL_NO_ITEMS WHERE SH_ID = @SH_ID";
        ...
        async Task<long> saveneweasyopenspecifications(SH_EASY_OPEN_DATA mydata)
        {
            if (f1_printing_stat.SelectedIndex == 0)
            {
                try
                {
                    string query = "INSERT INTO SH_SPECIFICATION_OF_EASY_OPEN ";
        ...
        async Task<long> saveneweasyopenquantities(long sp_id , SH_EASY_OPEN_DATA mydata)
        {
           
                try
                {
                    string query = "INSERT INTO SH_QUANTITY_OF_EASY_OPEN ";
                    query += "(SH_SPECIFICATION_OF_EASY_OPEN_ID, SH_ADDITION_PERMISSION_NUMBER, SH_ADDTION_DATE, SH_STOCK_ID, SH_STOCK_NAME, SH_STOCK_MAN_ID, SH_STOCK_MAN_NAME, SH_SUPPLY_DATE, SH_SUPPLIER_ID,";
                    ...
                    query += ") VALUES(@SH_SPECIFICATION_OF_EASY_OPEN_ID,@SH_ADDITION_PERMISSION_NUMBER,@SH_ADDTION_DATE,@SH_STOCK_ID,@SH_STOCK_NAME,@SH_STOCK_MAN_ID,@SH_STOCK_MAN_NAME,@SH_SUPPLY_DATE,@SH_SUPPLIER_ID,@SH_SUPPLIER_BRANCH_ID,@SH_SUPPLIER_BRANCH_NAME,@SH_SUPPLIER_NAME,@SH_CONTAINER_NAME,@SH_NO_ITEMS_PER_CONTAINER";
                    ...
                    myconnection.openConnection();
                    SqlCommand cmd = new SqlCommand(query, DatabaseConnection.mConnection);
                    cmd.Parameters.AddWithValue("@SH_SPECIFICATION_OF_EASY_OPEN_ID", sp_id);
        ...
        async Task  saveeasyopencontainers(long sp_id , long qu_id , SH_EASY_OPEN_DATA mydata)
        {
            try
            {
                //string query = "INSERT INTO SH_CONTAINER_OF_EASY_OPEN ";
                //query += "(SH_QUANTITY_OF_EASY_OPEN_ID, SH_SPECIFICATION_OF_EASY_OPEN_ID, SH_CONTAINER_NAME, SH_TOTAL_NO_ITEMS, SH_ADDITION_DATE";
                //query += " ,SH_SUBCONTAINER_NAME , SH_NO_OF_SUB_CONTAINER_PER_CONTAINER,SH_TOTAL_NUMBER_OF_SUB_CONTAINERS,SH_NO_ITEMS_PER_SUB_CONTAINER )VALUES(@SH_QUANTITY_OF_EASY_OPEN_ID,@SH_SPECIFICATION_OF_EASY_OPEN_ID,@SH_CONTAINER_NAME,@SH_TOTAL_NO_ITEMS,@SH_ADDITION_DATE,@SH_SUBCONTAINER_NAME , @SH_NO_OF_SUB_CONTAINER_PER_CONTAINER,@SH_TOTAL_NUMBER_OF_SUB_CONTAINERS,@SH_NO_ITEMS_PER_SUB_CONTAINER)";

                //myconnection.openConnection();
                //for (int i = 0; i < mydata.no_of_container; i++)
                //{
                //        SqlCommand cmd = new SqlCommand(query, DatabaseConnection.mConnection);
                //        cmd.Parameters.AddWithValue("@SH_QUANTITY_OF_EASY_OPEN_ID", qu_id);
                //        cmd.Parameters.AddWithValue("@SH_SPECIFICATION_OF_EASY_OPEN_ID", sp_id);
                //        cmd.Parameters.AddWithValue("@SH_CONTAINER_NAME", mydata.container_name);
                ...
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand("SH_SAVE_ESY_OPEN_CONTAINERS" , DatabaseConnection.mConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            easy_open_form_data_grid_view.Rows.Clear();
            if (form_data.Count > 0)
            {
                for (int i = 0; i < form_data.Count; i++)
                {
                    ...
                    easy_open_form_data_grid_view.Rows.Add(myparms);
                }
            }
        }
        ...
        {
            if (easy_open_form_data_grid_view.SelectedRows.Count > 0)
            {
                form_data.RemoveAt(easy_open_form_data_grid_view.SelectedRows[0].Index);
                filleasyopengridview();
            }
        }
