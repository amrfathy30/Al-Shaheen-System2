...
{
    public partial class addnewpeeloff : Form
    {

        List<SH_PEEL_OFF_DATA> form_data = new List<SH_PEEL_OFF_DATA>();
       ...
       async void fillsupplierbranchescombobox()
        ...
        void loadalleasyopenspecifications()
        {
            ...
            try
            {
                string query = "SELECT  SH_SPECIFICATION_OF_PEEL_OFF.* FROM SH_SPECIFICATION_OF_PEEL_OFF";
                DatabaseConnection myconnection = new DatabaseConnection();
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand(query , DatabaseConnection.mConnection);
                SqlDataReader reader = cmd.ExecuteReader();
        ...
        long checkifeasyopenspecificationsexistsornot( SH_PEEL_OFF_DATA mydata)
        {
            specifications.Clear();
        ...
        async Task updateeasyopenspecifications(long sp_id , SH_PEEL_OFF_DATA mydata)
        {
            try
            {
                string query = "UPDATE SH_SPECIFICATION_OF_PEEL_OFF SET SH_TOTAL_NO_ITEMS = SH_TOTAL_NO_ITEMS + @SH_TOTAL_NO_ITEMS WHERE SH_ID = @SH_ID";
        ...
        async Task<long> saveneweasyopenspecifications(SH_PEEL_OFF_DATA mydata)
        {
            if (f1_printing_stat.SelectedIndex == 0)
            {
                try
                {
                    string query = "INSERT INTO SH_SPECIFICATION_OF_PEEL_OFF ";
        ...
        async Task<long> saveneweasyopenquantities(long sp_id , SH_PEEL_OFF_DATA mydata)
        {
           
                try
                {
                    string query = "INSERT INTO SH_QUANTITY_OF_PEEL_OFF ";
                    query += "(SH_SPECIFICATION_OF_PEEL_OFF_ID, SH_ADDITION_PERMISSION_NUMBER, SH_ADDTION_DATE, SH_STOCK_ID, SH_STOCK_NAME, SH_STOCK_MAN_ID, SH_STOCK_MAN_NAME, SH_SUPPLY_DATE, SH_SUPPLIER_ID,";
                    ...
                    query += ") VALUES(@SH_SPECIFICATION_OF_PEEL_OFF_ID,@SH_ADDITION_PERMISSION_NUMBER,@SH_ADDTION_DATE,@SH_STOCK_ID,@SH_STOCK_NAME,@SH_STOCK_MAN_ID,@SH_STOCK_MAN_NAME,@SH_SUPPLY_DATE,@SH_SUPPLIER_ID,@SH_SUPPLIER_BRANCH_ID,@SH_SUPPLIER_BRANCH_NAME,@SH_SUPPLIER_NAME,@SH_CONTAINER_NAME,@SH_NO_ITEMS_PER_CONTAINER";
                    ...
                    myconnection.openConnection();
                    SqlCommand cmd = new SqlCommand(query, DatabaseConnection.mConnection);
                    cmd.Parameters.AddWithValue("@SH_SPECIFICATION_OF_PEEL_OFF_ID", sp_id);
        ...
        async Task  saveeasyopencontainers(long sp_id , long qu_id , SH_PEEL_OFF_DATA mydata)
        {
            try
            {
                //string query = "INSERT INTO SH_CONTAINER_OF_PEEL_OFF ";
                //query += "(SH_QUANTITY_OF_PEEL_OFF_ID, SH_SPECIFICATION_OF_PEEL_OFF_ID, SH_CONTAINER_NAME, SH_TOTAL_NO_ITEMS, SH_ADDITION_DATE";
                //query += " ,SH_SUBCONTAINER_NAME , SH_NO_OF_SUB_CONTAINER_PER_CONTAINER,SH_TOTAL_NUMBER_OF_SUB_CONTAINERS,SH_NO_ITEMS_PER_SUB_CONTAINER )VALUES(@SH_QUANTITY_OF_PEEL_OFF_ID,@SH_SPECIFICATION_OF_PEEL_OFF_ID,@SH_CONTAINER_NAME,@SH_TOTAL_NO_ITEMS,@SH_ADDITION_DATE,@SH_SUBCONTAINER_NAME , @SH_NO_OF_SUB_CONTAINER_PER_CONTAINER,@SH_TOTAL_NUMBER_OF_SUB_CONTAINERS,@SH_NO_ITEMS_PER_SUB_CONTAINER)";

                //myconnection.openConnection();
                //for (int i = 0; i < mydata.no_of_container; i++)
                //{
                //        SqlCommand cmd = new SqlCommand(query, DatabaseConnection.mConnection);
                //        cmd.Parameters.AddWithValue("@SH_QUANTITY_OF_PEEL_OFF_ID", qu_id);
                //        cmd.Parameters.AddWithValue("@SH_SPECIFICATION_OF_PEEL_OFF_ID", sp_id);
                //        cmd.Parameters.AddWithValue("@SH_CONTAINER_NAME", mydata.container_name);
                ...
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand("SH_SAVE_PEEL_OFF_CONTAINERS", DatabaseConnection.mConnection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            PEEL_OFF_form_data_grid_view.Rows.Clear();
            if (form_data.Count > 0)
            {
                for (int i = 0; i < form_data.Count; i++)
                {
                    ...
                    PEEL_OFF_form_data_grid_view.Rows.Add(myparms);
                }
            }
        }
        ...
        {
            if (PEEL_OFF_form_data_grid_view.SelectedRows.Count > 0)
            {
                form_data.RemoveAt(PEEL_OFF_form_data_grid_view.SelectedRows[0].Index);
                filleasyopengridview();
            }
        }
