...
{
    public partial class pallets_portal_form : Form
    {
        SH_EMPLOYEES mEmployee;
        SH_USER_ACCOUNTS mAccount;
        SH_USER_PERMISIONS mPermission;
        public pallets_portal_form(SH_EMPLOYEES anyemp , SH_USER_ACCOUNTS anyAccount , SH_USER_PERMISIONS anyPerm)
        {
            InitializeComponent();
            mEmployee = anyemp;
            mAccount = anyAccount;
        ...
        private void listView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            switch (currentitem.Index)
            {
                case 0:
                    {
                        pallet_data_profile myform = new pallet_data_profile("110*120",120,110,mEmployee,mAccount,mPermission);
                        myform.Show();
                        break;
                    }
                case 1:
                    {
                        pallet_data_profile myform = new pallet_data_profile("110*130",130,110, mEmployee, mAccount, mPermission);
                        myform.Show();
                        break;
                    }
                case 2:
                    {
                        pallet_data_profile myform = new pallet_data_profile("90*135", 135, 90, mEmployee, mAccount, mPermission);
                        myform.Show();
                        break;
                    }
                case 3:
                    {
                        pallet_data_profile myform = new pallet_data_profile("80*100", 100, 80, mEmployee, mAccount, mPermission);
                        myform.Show();
                        break;
                    }
                default:
