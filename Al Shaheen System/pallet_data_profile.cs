...
{
    public partial class pallet_data_profile : Form
    {
        ...
        public pallet_data_profile(string pallet_size , long anylength,long anywidth , SH_EMPLOYEES anyemp , SH_USER_ACCOUNTS anyaccount , SH_USER_PERMISIONS anypermission )
        {
            InitializeComponent();
            mEmployee = anyemp;
            mAccount = anyaccount;
        ...
        async Task loadallpalletssuppliers()
        {
            suppliers.Clear();
            try
            {
                ...
                query += " WHERE SITM.SH_ITEM_NAME LIKE '%بالتة%' ";
                query += " OR SITM.SH_ITEM_NAME LIKE '%بالته%' ";
                query += " OR SITM.SH_ITEM_NAME LIKE '%بالتات%'";
                SqlCommand cmd = new SqlCommand(query , DatabaseConnection.mConnection);
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
        ...
        async Task getstocktotalquantities()
        {
            ...
            {
                myconnection.openConnection();
                string query = "SELECT COUNT(ADQP.SH_ID) AS TOTAL_QUANTITY FROM sh_individual_pallets  ADQP  ";
                query += " LEFT JOIN SH_ADDED_QUANTITY_OF_PALLETS AQP ON ";
                query += " AQP.SH_ID = ADQP.SH_ADDED_QUANTITY_OF_PALLETS_ID ";
                query += " WHERE ADQP.SH_ID not in (select SH_INDIVIDUAL_PALLETS_ID ";
                query += " from SH_EXCHANGE_OF_PALLETS ) AND ";
        ...
        async Task fillclientsrecordsgridview()
        {
            try
            {
                string query = "select (SUM(SH_NO_PALLETS) - (select SUM(SH_QUANTITY_NO_ITEMS) as retured_quantities from SH_ADDED_QUANTITY_OF_PALLETS where SH_CLIENT_ID = SH_CLIENT_ID and SH_SIZE_LENGTH = 120 and SH_SIZE_WIDTH = 110)) as pallets ,SUM(SH_NO_WOOD_WINCHES) as wooden_top , ";
        ...
        private async void pallet_data_profile_Load(object sender, EventArgs e)
        {
            ...
            await autogeneratepalletsreturnrequistnumber();
            await getstocktotalquantities();
            await loadallstocks();
            await loadallpalletssuppliers();
        ...
        async Task autogeneratebottomaddtion_permission_number()
        {
            ...
            {
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand("SELECT (MAX(SH_ID)+1) AS lastedid FROM SH_PALLETS_ADDITION_PERMISSION_NUMBER  ", DatabaseConnection.mConnection);
                SqlDataReader reader = cmd.ExecuteReader();
        ...
        private void savenewpermssionnumber()
        {
            try
            {
                ...
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand("INSERT INTO SH_PALLETS_ADDITION_PERMISSION_NUMBER (SH_NUMBER) VALUES(@SH_NUMBER) ", DatabaseConnection.mConnection);
        ...
        async Task autogeneratepalletreturnpermissionnumber()
        {
            ...
            if (mycount == 0)
            {
                //there is the first rows in the db
                string permissionnumber = "SH_";
                permissionnumber += "RETURN_PALLET-";
                permissionnumber += DateTime.Now.ToString("yy");
                string currentr = 1.ToString();
                for (int i = 0; i < 5 - 1; i++)
                {
        ...
        async Task autogeneratepalletsreturnrequistnumber()
        {
            long mycount = 0;
            try
            {
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand("SELECT (MAX(SH_ID)+1) AS lastedid FROM SH_PALLETS_RETURN_REQUIST_NUMBER  ", DatabaseConnection.mConnection);
                SqlDataReader reader = cmd.ExecuteReader();
        ...
        private void savenewreturnrequistnumber()
        {
            try
            {
                ...
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand("INSERT INTO SH_PALLETS_RETURN_REQUIST_NUMBER (SH_NUMBER) VALUES(@SH_NUMBER) ", DatabaseConnection.mConnection);
                cmd.Parameters.AddWithValue("@SH_NUMBER", 1.ToString());
                cmd.ExecuteNonQuery();
                myconnection.closeConnection();
            }
        ...
        async Task loadallpalletsspecifications()
        {
            pallets_specifications.Clear();
            try
            {
                myconnection.openConnection();

                SqlCommand cmd = new SqlCommand("SELECT * FROM SH_SPECIFICATION_OF_PALLETS ", DatabaseConnection.mConnection);
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    pallets_specifications.Add(new SH_SPECIFICATION_OF_PALLETS() {
                        SH_ID = long.Parse(reader["SH_ID"].ToString()),
                        SH_PALLET_LENGTH = long.Parse(reader["SH_PALLET_LENGTH"].ToString()),
                        SH_PALLET_WIDTH = long.Parse(reader["SH_PALLET_WIDTH"].ToString()),
            ...
            {
                MessageBox.Show("ERROR WHILE GETTING PALLETS SPECIFICATIONS "+ex.ToString());
            }
        }
        async Task<long> check_if_pallet_specification_exists_or_not()
        {
            if (pallets_specifications.Count>0)
            {
                for (int i = 0; i < pallets_specifications.Count; i++)
                {
                    if (string.Compare(pallets_specifications[i].SH_PALLET_SIZE_TEXT,pallet_size_text_box.Text)==0 )
                    {
                        return pallets_specifications[i].SH_ID;
                    }
                }
            }
            return 0;
        }
        async Task <long> savenewpalletspecification(long quantity )
        {
            try
            {
                myconnection.openConnection();
                string query = " INSERT INTO SH_SPECIFICATION_OF_PALLETS ";
                query += " (SH_PALLET_LENGTH,  SH_PALLET_WIDTH, SH_TOTAL_QUANTITY, SH_PALLET_SIZE_TEXT) ";
                query += " VALUES(@SH_PALLET_LENGTH,  @SH_PALLET_WIDTH, @SH_TOTAL_QUANTITY, @SH_PALLET_SIZE_TEXT)";
                query += " SELECT SCOPE_IDENTITY() AS myidentity ";
                SqlCommand cmd = new SqlCommand(query, DatabaseConnection.mConnection);
                cmd.Parameters.AddWithValue("@SH_PALLET_LENGTH", p_length);              
                cmd.Parameters.AddWithValue("@SH_PALLET_WIDTH", p_width);
                cmd.Parameters.AddWithValue("@SH_TOTAL_QUANTITY", quantity);
                cmd.Parameters.AddWithValue("@SH_PALLET_SIZE_TEXT", pallet_size_label.Text);
                SqlDataReader reader = cmd.ExecuteReader();
                long myid = 0;
                if (reader.Read())
                {
        ...
        async Task<long> saveexaminationofpallets(string examination_number , long m_one , long m_two)
        {
            try
            {
                myconnection.openConnection();
                SqlCommand cmd = new SqlCommand("SH_SAVE_NEW_EXAMINATION_RECORD_OF_PALLETS" , DatabaseConnection.mConnection);
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                MessageBox.Show("ERROR WHIlE SAVING EXAMINATION OF PALLETS "+ex.ToString());
            }
            return 0;
        }
        async Task  savepalletsquantities(long sp_id,long quantity)
        {
            long ex_id = await saveexaminationofpallets(examination_record_number_text_box.Text , stock_men[monitors_one.SelectedIndex].SH_ID , stock_men[monitors_two.SelectedIndex].SH_ID);
            if (ex_id == 0)
            {
            ...
            else
            {
                ...
                {
                    myconnection.openConnection();
                    string query = "INSERT INTO SH_ADDED_QUANTITY_OF_PALLETS ";
                    query += " ( SH_SPECIFICATION_OF_PALLETS_ID ";
                    query += " ,SH_EXAMINATION_RECORD_OF_PALLETS_ID ";
                    ...
                    cmd.Parameters.AddWithValue("@SH_SPECIFICATION_OF_PALLETS_ID", sp_id);
                    cmd.Parameters.AddWithValue("@SH_PALLET_SIZE_TEXT", pallet_size_label.Text);
                    cmd.Parameters.AddWithValue("@SH_EXAMINATION_RECORD_OF_PALLETS_ID", ex_id);
                    long myid = 0;
                    SqlDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
                    ...
                    reader.Close();
                    myconnection.closeConnection();
                    await savenewindividaulpallets(myid, quantity);

                }
                catch (Exception ex)
                {
                    MessageBox.Show("ERROR WHILE ADDING NEW PALLETS QUANTITIES " + ex.ToString());
                }
            }
           
        }

        async Task savepalletsreturnquantities(long sp_id , long quantity)
        {
            long ex_id = await saveexaminationofpallets(examination_record_number_textbox.Text , stock_men[monitor_one_combo_box.SelectedIndex].SH_ID , stock_men[monitor_two_combo_box.SelectedIndex].SH_ID);
            if (ex_id == 0)
            {
                MessageBox.Show("  لايمكن حفظ الكمية لعدم وجود محضر فحص ", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1, MessageBoxOptions.RtlReading);
            }
            else
            {
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("@SH_PALLETS_RETURN_REQUIST_NUMBER", client_return_permission_number.Text);
                    long myid = 0;
                    SqlDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
        ...
        async Task  updatepalletsspecification(long sp_id , long quantity)
        {
            try
            {
                myconnection.openConnection();
                string query = "UPDATE SH_SPECIFICATION_OF_PALLETS ";
                query += " SET ";      
        ...
        async Task savenewindividaulpallets(long qu_id ,long quantity)
        {
            try
            {
                myconnection.openConnection();
                    SqlCommand cmd = new SqlCommand("SH_SAVE_PALLET_INDIVIDUALS_DATA", DatabaseConnection.mConnection);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@SH_QUANTITY_NUMBER", quantity);
                cmd.Parameters.AddWithValue("@SH_ADDED_QUANTITY_OF_PALLETS_ID", qu_id);
                cmd.Parameters.AddWithValue("@SH_PALLET_SIZE_TEXT", pallet_size_label.Text);
                cmd.ExecuteNonQuery();
                myconnection.closeConnection();
            }
        ...
        private async void save_new_pallets_quantity_btn_Click(object sender, EventArgs e)
        {
            ...
            if (cansave)
            {
                await loadallpalletsspecifications();
                long sp_id = await check_if_pallet_specification_exists_or_not();
                if (sp_id==0)
                {
                    sp_id = await savenewpalletspecification(long.Parse(quantity_combo_box.Text));
                     await savepalletsquantities(sp_id , long.Parse(quantity_combo_box.Text));
                  
                }else
                {
                    await updatepalletsspecification(sp_id, long.Parse(quantity_combo_box.Text));
                    await savepalletsquantities(sp_id, long.Parse(quantity_combo_box.Text));
        ...
        private async void saverequist_andprint_btn_Click(object sender, EventArgs e)
        {
            ...
            if (cansave)
            {
                ...
                {
                    myconnection.openConnection();
                    string query = "INSERT INTO SH_PALLETS_RETURN_REQUIST ";
                    query += " (SH_DATA_ENTRY_EMPLOYEE_ID, ";
                    query += "  SH_DATA_ENTRY_USER_ID, SH_ADDTION_DATE, ";
                    query += " SH_CLIENT_ID, SH_DRIVER_NAME, ";
                    query += " SH_PALLETS_RETURN_REQUIST_NUMBER) ";
                    ...
                    cmd.Parameters.AddWithValue("@SH_PALLETS_RETURN_REQUIST_NUMBER", return_requist_number_text_box.Text);
        ...
        private async void save_return_pallet_btn_Click(object sender, EventArgs e)
        {
            ...
            if (cansave)
            {
                ...
                if (sp_id == 0)
                {
                    sp_id = await savenewpalletspecification(long.Parse(quantity_text_box.Text));
                    await savepalletsreturnquantities(sp_id, long.Parse(quantity_text_box.Text));

                }
                else
                {
                    await updatepalletsspecification(sp_id, long.Parse(quantity_text_box.Text));
                    await savepalletsreturnquantities(sp_id , long.Parse(quantity_text_box.Text));
